...
ServerCtrl.$inject = ['$scope','$route','$http','$interval','$routeParams','_','Helpers','Oboe','$mdToast','$q'];

function ServerCtrl($scope,$route,$http,$interval,$routeParams,_,Helpers,Oboe,$mdToast,$q) {
		var vm = this;
		vm.title = '';
		...
		var call_getcustomermetrics = $http.get('/getcustomermetrics/'+$routeParams.servername+'/'+$routeParams.alias+'/'+$routeParams.db);
		var call_getlicenses = $http.get('/getlicenses/'+$routeParams.alias+'/'+vm.db);
		var call_getdiskspace = $http.get('/getdiskspace/'+$routeParams.alias+'/'+vm.db);
		var call_gettop10errors = $http.get('/gettop10errors/'+$routeParams.alias+'/'+vm.db);
		$q.all([call_getcustomermetrics, call_getlicenses, call_getdiskspace, call_gettop10errors])
				.then(function(data) {
				//.success(function(data) {
						// console.log(data);

						// customermetrics
						data[0].data = Helpers.parseTimestamps(data[0].data);

						vm.mockdata = data[0].data;
						vm.mockdata.reverse();
						...
						_.each(data[0],function(obj,index){
						...
						// licenses
						data[1].data = Helpers.parseTimestamps(data[1].data);
						vm.licensesstarting = false;
						vm.licenses = data[1].data;

						// diskspace
						data[2].data = Helpers.parseTimestamps(data[2].data);
						vm.diskspacestarting = false;
						vm.diskspace = data[2].data;

						// top10errors
						data[3].data = Helpers.parseTimestamps(data[3].data);
						vm.top10errorsstarting = false;
						vm.top10errors = data[3].data;
		...
		vm.getfailedjobs = function(){
				$http.get('/getfailedjobs/'+$routeParams.alias+'/'+$routeParams.db)
					.success(function(data) {
						vm.getfailedjobs = null;
						vm.getfailedjobs = data;
					})
					.error(function(data) {
							console.log('Error: ' + data); 
							// vm.error = data;
